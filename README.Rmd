...
The modifications and additions in `{samplelim}` with respect to  `{volesti}` 1.1.2-6 include:

  * the removal of the bound on the number of reflections for the BiW ;
  * the introduction of the MiW. Its implementation in `{samplelim}` is an optimized, C++ encoded version of the algorithm implemented in pure R programming language in `{limsolve}` ;
  * support functions inspired by `{limsolve}` that allow users an easy handling.
