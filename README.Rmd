...
The package `{samplelim}` provides performant implementations (C++ encoded) of several Monte Carlo Markov Chains (MCMC) methods for uniformly sampling hig-dimensional polytopes. 
...
`{samplelim}` aims at providing performant implementations of two MCMC algorithms for sampling high-dimensional polytopes; namely, the Mirror Walk (MiW) introduced by ..... and the Billard Walk (BiW) introduced by Chalkis _et al._ (2014). It also provides support functions to ease its use by ecologists practitioners interested in solving LIM for trophic networks. Hence, `{samplelim}` can be viewed as an updated, extended and low-level-encoded version of the R package [`{limsolve}`](https://cran.r-project.org/web/packages/limSolve/index.html).
...
The Declaration Files in `inst/extdata` have been produced by Quentin NOGUÈS.
