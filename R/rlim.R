...
#'
#' @param lim A list with four components \code{A}, \code{B}, \code{G} and \code{H} representing the polytope to be sampled; 
#' see the section \emph{Details} below.
#' @param Hpol A Hpolytope object representing the reduced polytope to be sampled. If \code{NULL} (the default), it is computed from \env{lim}.
...
#' @param jmp A numeric value (if \env{type} is \code{"BiW"}) or a numeric vector (if \env{type} is \code{"MiW"}) giving the jump rate parameter of the MCMC algorithm to be used. 
#' If \code{NULL} (the default), it is automatically computed from \env{lim}; see the section \emph{Details} below.
...
#' @param nsamp An integer; the length of the output sample of points into the polytope. Default is 3000.
...
#' \itemize{
#'   \item the dimension of the inner space the polytope relies into (i.e., the number of flows), if Hpol is NULL;
#'   \item the dimension of Hpol is Hpol is an object of class Hpolytope.
#'  }
#' @export
#'
#' @details
#' A polytope \eqn{\mathcal{P}} is a convex subset or \eqn{\mathbb{R}^n}, \eqn{n \in \mathbb{N^*}}, defined as the intersection of hyper-planes and half-spaces. 
#' More precisely, \eqn{\mathcal{P} = \{ x \in \mathbb{R}^n: Ax = B, Gx \geq H \}}, 
#' where \eqn{A} is an \eqn{m\times n} matrix, with \eqn{m \leq n}, \eqn{B \in \mathbb{R}^m}, \eqn{G} is a \eqn{k \times n} matrix and \eqn{H \in \mathbb{R}^k}.
#' For polytopes involved in Linear Inverse Models (LIM), \eqn{n} is the number of flows in the metabolic network. The \eqn{k} inequality constraints insures that \eqn{\mathcal{P}} is bounded. 
#' The polytope to be sampled is given to \code{rlim()} whether by:
#' \itemize{
#'   \item creating a list with four elements A, B, G, H, giving the set of equality and inequality constraints for the polytope, and using it for \code{lim} argument. 
#' This list can be created manually or by importing a LIM declaration file thanks to the function \code{\link{df2lim}()}. Or by
#'   \item creating an object of class Hypolytope (originating from package \pkg{volesti}) thanks to the function \code{Hpolytope()}; see \link{Hpolytope-class}. 
#' This Hypolytope object is passed to argument \code{Hpol}, and requires argument \code{lim} to be set to \code{NULL} for being processed. 
#' }
#' 
#' The main practical difference between lim and Hpolytope objects is that Hypolytope allows for inequality constraints only, 
#' hence the polytope is of full dimension \eqn{n}. 
#'  
#' 
#' 
...
#' Both are reflective Hamiltonian algorithms: each new point of the sample is generated from the previous one by
#' choosing randomly a direction and moving forward this direction for a random length. 
#' If the end point of this segment belongs to the polytope, then it is added to the sample; 
#' if not, then, the trajectory is reflected on borders of polytope until the end point belongs to the polytope.
#' BiW and MiW differ in the probability distributions used for generating the length of the trajectory; see \cite{Girardin at al. (2023)} for details.
#' Still, these distributions depend on a parameter called (in both cases) the jump rate of the trajectory.
#' 
#' The jump rate has a strong influence on the performance of the algorithm: 
#' too small, a very large number of points \env{nsim} would be needed to efficiently sample the polytope. 
#' Too large, the trajectories reflect an important number of times before resulting in a point inside the polytope, making the process very slow.
#' If \env{jmp} is \code{NULL} (default), it is computed automatically, depending the \env{type} (MiW or BiW). Precisely,
#' \itemize{
#'  \item if \env{type} is \code{"MiW"}, then \env{jmp} is a random vector of length equal to the dimension of the reduced polytope. 
#'  It is equal to the ranges of the reduced polytope divided by \env{scale} (default for \env{scale} is 10);
#'  \item if \env{type} is \code{"BiW"}, the \env{jmp} is a single value equal to the radius of the largest ball included in the reduced polytope.
#' }
#' 
#' @examples
...
#' # Default behaviour is to set the size nsamp of the returned sample, taking into account burn-in and thinning.
...
#' all(sample1 == sample2)
#' @seealso \code{\link{df2lim}()} for creating an lim object from a declaration file, \link{Hpolytope-class} for class Hpolytope.
#' @references {
#' D. Van Oevelen, K. Van den Meersche, F. J. R. Meysman, K. Soetaert, J. J. Middelburg and A. F. Vézina, 
#' _Quantifying Food Web Flows Using Linear Inverse Models_, Ecosystems __13__, 32-45 (2010).
#' 
#' B.T. Polyak and E.N. Gryazina, 
#' _Billiard walk - a new sampling algorithm for control and optimization_, IFAC Proceedings Volumes, __47(3)__, 6123-6128 (2014).
#' 
#' V. Girardin, T. Grente, N. Niquil and P. Regnault,
#' _Comparing and updating R packages of MCMC Algorithms for Linear Inverse Modeling of Metabolic Networks_,
#' hal:    (2023)
#' }
